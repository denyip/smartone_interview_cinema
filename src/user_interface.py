...
class UserInterface:
    ...
    def get_movie_and_seats(self):
        ...
        while True:
            try:
                ...
                self.movie_and_seats = MovieAndSeats(
                    title, rows, seats_per_row)
                break
            except IndexError:
    ...
    def main_menu(self):
        ...
        print(
            f"Movie title: {movie_and_seats.title}, Rows: {movie_and_seats.rows}, Seats per row: {movie_and_seats.seats_per_row}")
        while True:
        ...
        while True:
            try:
                ...
                if num_of_tickets > movie_and_seats.available_seats:
                    ...
                    continue
                target_seating_map = [row[:]
                                      for row in movie_and_seats.seating_map]
                booking_services = BookingServices(
                    movie_and_seats, num_of_tickets, target_seating_map)
                ...
                print(
                    f"Successfully reserved {num_of_tickets} {movie_and_seats.title} tickets.")
                print(f"Booking ID: {booking_id}")
                self.display_seating_map(target_seating_map)
                new_postion = input(
                    "Enter blank to accept seat selection, or enter a new seating position\n>")
                if not new_postion:
                    ...
                    while True:
                        ...
                        booking_services = BookingServices(
                            movie_and_seats, num_of_tickets, target_seating_map)
                        best_seats = booking_services.find_the_best_seats_specific_start_seat(
                            new_postion)
                        for seat in best_seats:
                            row, col = seat
                            target_seating_map[row][col] = SELECTED_SEAT_MARK
    ...
    def display_seating_map(self, seating_map: List[List[str]]):
        ...
        screen_text_padding = SPACE_CHAR * \
            ((total_width - len(SCREEN_TEXT)) // 2)
